...
namespace Inflatable.Aspect.EndMethod
{
    ...
    public class IEnumerableProperty : IEndMethodHelper
    {
        /// <summary>
        ...
        public void Setup(string returnValueName, MethodInfo method, IMapping mapping, StringBuilder builder)
        {
            ...
            Builder.AppendLineFormat("if(!{0}&&Session0!=null)", Property.InternalFieldName + "Loaded")
                ...
                .AppendLineFormat("((ObservableList<{1}>){0}).ForEach(TempObject => {{ ((IORMObject)TempObject).PropertyChanged += (x, y) => ((ObservableList<{1}>){0}).NotifyObjectChanged(x); }});", Property.InternalFieldName, Property.TypeName)
                .AppendLine("}")
