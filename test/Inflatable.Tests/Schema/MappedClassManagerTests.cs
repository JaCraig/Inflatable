...
namespace Inflatable.Tests.Schema
{
    public class MappedClassManagerTests : TestingFixture
    {
        ...
        public void Creation()
        {
            ...
            Assert.Contains("CREATE TABLE [dbo].[MapProperties_]([ID_] Int NOT NULL PRIMARY KEY IDENTITY,[BoolValue_] Bit NOT NULL,[MappedClass_AllReferencesAndID_ID_] Int)", TestModel.GeneratedSchemaChanges);
            Assert.Contains("ALTER TABLE [dbo].[MapProperties_] ADD FOREIGN KEY ([MappedClass_AllReferencesAndID_ID_]) REFERENCES [dbo].[AllReferencesAndID_]([ID_]) ON DELETE SET NULL", TestModel.GeneratedSchemaChanges);
        }
    }
}